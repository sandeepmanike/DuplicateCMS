...
[Migration("20260810113000_FinalRepairCertificatesReportsAndAdmissionSchema")]
public partial class FinalRepairCertificatesReportsAndAdmissionSchema : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        // This repair migration is intentionally idempotent. It fixes databases where
        // EF migration history says the module is applied but the physical objects are missing.
...
) CHARACTER SET=utf8mb4;", suppressTransaction: true);
...
CREATE TABLE IF NOT EXISTS `Certificates` (
  ...
  KEY `IX_Certificates_Status` (`Status`)
        ...
        // Ensure the Certificates foreign key exists even if the table was created by an older script.
        migrationBuilder.Sql(@"
SET @exists_fk := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='Certificates'
    AND CONSTRAINT_NAME='FK_Certificates_Students_StudentId'
);
SET @sql := IF(@exists_fk=0,
  'ALTER TABLE `Certificates` ADD CONSTRAINT `FK_Certificates_Students_StudentId` FOREIGN KEY (`StudentId`) REFERENCES `Students` (`StudentId`) ON DELETE RESTRICT ON UPDATE CASCADE',
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;", suppressTransaction: true);

        // Add the admission link if an older Students table does not have it.
        migrationBuilder.Sql(@"
SET @exists_col := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='Students' AND COLUMN_NAME='AdmissionId'
);
SET @sql := IF(@exists_col=0,
  'ALTER TABLE `Students` ADD COLUMN `AdmissionId` INT NULL AFTER `StudentId`',
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;", suppressTransaction: true);

        // Remove the legacy field which is not part of the frontend admission flow.
        migrationBuilder.Sql(@"
SET @exists_col := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='Students' AND COLUMN_NAME='PreviousHallTicketNumber'
);
SET @sql := IF(@exists_col>0,
  'ALTER TABLE `Students` DROP COLUMN `PreviousHallTicketNumber`',
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;", suppressTransaction: true);

        // Avoid duplicate AdmissionId values before adding the unique index.
        migrationBuilder.Sql(@"
UPDATE `Students` s
JOIN (
...
WHERE s.StudentId<>d.KeepStudentId;", suppressTransaction: true);

        migrationBuilder.Sql(@"
SET @exists_idx := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='Students' AND INDEX_NAME='UX_Students_AdmissionId'
);
SET @sql := IF(@exists_idx=0,
  'CREATE UNIQUE INDEX `UX_Students_AdmissionId` ON `Students` (`AdmissionId`)',
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;", suppressTransaction: true);
...
SET @exists_fk := (
  SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA=DATABASE() AND TABLE_NAME='Students'
    AND CONSTRAINT_NAME='FK_Students_StudentAdmissions_AdmissionId'
);
SET @sql := IF(@exists_fk=0,
  'ALTER TABLE `Students` ADD CONSTRAINT `FK_Students_StudentAdmissions_AdmissionId` FOREIGN KEY (`AdmissionId`) REFERENCES `StudentAdmissions` (`AdmissionId`) ON DELETE RESTRICT ON UPDATE CASCADE',
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;", suppressTransaction: true);
...
BEGIN SELECT CertificateId,CertificateNumber,StudentId,AdmissionNo,StudentName,GroupName,AcademicLevel,AcademicYear,CertificateType,Purpose,Remarks,Status,GeneratedAt,ReviewedAt,ApprovedAt,IssuedAt,IssuedBy,IsActive FROM Certificates WHERE CertificateId=p_CertificateId; END;", suppressTransaction: true);

        var generationSql = new[]
        {
            ("sp_GenerateBonafideCertificate", "Bonafide Certificate"),
            ("sp_GenerateStudyCertificate", "Study Certificate"),
            ("sp_GenerateConductCertificate", "Conduct Certificate"),
            ("sp_GenerateFeeCertificate", "Fee Certificate"),
            ("sp_GenerateTransferCertificate", "Transfer Certificate")
        };
        foreach (var (procedure, type) in generationSql)
        {
            migrationBuilder.Sql($@"
DROP PROCEDURE IF EXISTS {procedure};
CREATE PROCEDURE {procedure}(IN p_StudentId INT,IN p_Purpose VARCHAR(250),IN p_IssueDate DATETIME,IN p_Remarks VARCHAR(1000))
BEGIN
 ...
 INSERT INTO Certificates(CertificateNumber,StudentId,AdmissionNo,StudentName,GroupName,AcademicLevel,AcademicYear,CertificateType,Purpose,Remarks,Status,GeneratedAt,IssuedAt,IsActive) VALUES(v_Number,p_StudentId,v_AdmissionNo,v_StudentName,v_GroupName,v_Level,v_Year,'{type}',TRIM(p_Purpose),NULLIF(TRIM(p_Remarks),''),'Generated',CURRENT_TIMESTAMP(6),p_IssueDate,1);
 SET v_CertificateId=LAST_INSERT_ID();
 INSERT INTO AuditLogs(UserName,Action,EntityName,EntityId,Description) VALUES(NULL,'GENERATE','Certificate',v_CertificateId,CONCAT('{type} generated for ',v_StudentName));
...
BEGIN SELECT CertificateId,CertificateNumber,StudentId,AdmissionNo,StudentName,GroupName,AcademicLevel,AcademicYear,CertificateType,Purpose,Remarks,Status,GeneratedAt,ReviewedAt,ApprovedAt,IssuedAt,IssuedBy,IsActive FROM Certificates WHERE (p_StudentId IS NULL OR StudentId=p_StudentId) ORDER BY CertificateId DESC; END;", suppressTransaction: true);

        migrationBuilder.Sql(@"
DROP PROCEDURE IF EXISTS sp_VerifyCertificate;
CREATE PROCEDURE sp_VerifyCertificate(IN p_CertificateNumber VARCHAR(40))
BEGIN SELECT CertificateId,CertificateNumber,StudentId,AdmissionNo,StudentName,GroupName,AcademicLevel,AcademicYear,CertificateType,Purpose,Remarks,Status,GeneratedAt,ReviewedAt,ApprovedAt,IssuedAt,IssuedBy,IsActive FROM Certificates WHERE CertificateNumber=p_CertificateNumber AND IsActive=1 AND Status<>'Cancelled'; END;", suppressTransaction: true);
...
END;", suppressTransaction: true);
    ...
    {
        // Intentionally left empty. This migration repairs missing schema objects and
        // must never delete existing Certificates, AuditLogs, or stored procedures on rollback.
    }
}
